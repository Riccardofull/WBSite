                ...
                <div v-if="open" class="absolute z-10 top-0 inset-x-0 p-2 transition transform origin-top-right md:hidden">
                    <div class="rounded-lg shadow-md bg-white ring-1 ring-black ring-opacity-5 overflow-hidden">
                        <div class="px-5 pt-4 flex items-center justify-between">
                            ...
                            <div class="-mr-2">
                            <button @click="open = false" type="button" class="bg-white rounded-md p-2 inline-flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-inset">
                                <svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                        ...
                        <div class="px-2 pt-2 pb-3 space-y-1">
                            <a href="/" class="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-gray-900 hover: cursor-pointer" :class="$route.path == '/' ? 'font-semibold' : 'font-medium'">{{$t("home")}}</a>
                            <a @click="$router.push({ path: `/drawings`, query: { category: 'BlackAndWhite' } })" class="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-gray-900 hover: cursor-pointer" :class="$route.path == '/drawings' ? 'font-semibold' : 'font-medium'">{{$t("drawings")}}</a>
                            <a href="/contacts" class="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-gray-900 hover: cursor-pointer" :class="$route.path == '/contacts' ? 'font-semibold' : 'font-medium'">{{$t("contacts")}}</a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <main class="mt-16 mx-auto max-w-7xl px-4 sm:mt-24">
                <div class="text-center">
                    <h2 class="text-4xl tracking-tight font-extrabold text-gray-900 sm:text-5xl md:text-6xl">
                        <span class="block xl:inline">{{$t(title)}}</span>
                    </h2>
